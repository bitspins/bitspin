...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "SpinCoins is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "SpinCoins is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "SpinCoins is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "SpinCoins is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
   ...
   // result.push_back(Pair("charityvalue", (int64_t)pblock->vtx[0].vout[0].nValue));  // SPC un
   // result.push_back(Pair("target", hashTarget.GetHex())); // SPC un
